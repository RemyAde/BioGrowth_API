...
@router.post("/create")
async def create_product_endpoint(product_request: product_pydanticIn, plan_id: int, user: user_pydantic = Depends(get_admin_user)): # type: ignore
...
async def create_upload_file(product_id: int, file: UploadFile = File(...),
                             user: user_pydantic = Depends(get_admin_user)): # type: ignore
...
async def edit_product(product_id: int, update_dict: product_pydanticIn, user: user_pydantic = Depends(get_admin_user)): # type: ignore
...
async def delete_product(product_id: int, user: user_pydantic = Depends(get_admin_user)): # type: ignore
    product = await Product.get(id = product_id)
    if product:
